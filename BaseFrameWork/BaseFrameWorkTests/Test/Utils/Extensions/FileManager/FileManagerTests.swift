//
//  FileManagerTests.swift
//  BaseFrameWorkTests
//
//  Created by ZhangYu on 2021/8/30.
//

import XCTest

class FileManagerTests: XCTestCase {

    override func setUpWithError() throws {
        // Put setup code here. This method is called before the invocation of each test method in the class.
    }

    override func tearDownWithError() throws {
        // Put teardown code here. This method is called after the invocation of each test method in the class.
    }

    func testExample() throws {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
    }

    func testPerformanceExample() throws {
        // This is an example of a performance test case.
        self.measure {
            // Put the code you want to measure the time of here.
        }
    }

}

// MARK: - Getter

extension FileManagerTests {

    func testDocument() {
        print("\(ConsoleDebugPrefix) FileManager.document is:\(FileManager.document)")
        
        XCTAssertEqual(FileManager.document, FileManager.default.document)
    }

}

// MARK: - Create

extension FileManagerTests {

    func testCreateDirectory() {
        let directoryURL = FileManager.default.document
        let dirURL = directoryURL.appendingPathComponent("demo", isDirectory: true)
        do {
            try FileManager.createDirectory(at: dirURL)
            print("\(ConsoleDebugPrefix) dirURL is:\(dirURL)")

            XCTAssertTrue(FileManager.default.fileExists(atPath: dirURL.path))
        } catch {
            XCTFail("directory should be created")
        }
    }

}
